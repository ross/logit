...
The first three came out of one `script/record-fixtures collectd` run's first capture, against
...
listener, its own `docker run`, against `tools/record-fixtures/collectd-threshold.conf` (a copy of
`collectd.conf` plus the `threshold` plugin, kept as its own file rather than shared so the first
capture never loads `threshold` too) -- kept apart specifically so it never perturbs the first
three (see that row's own entry, and `record_collectd`'s comment in `script/record-fixtures`, for
why).
