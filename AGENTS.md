...
| `script/validate` | Manually run `logit validate` over every shipped config (`demo/`, `examples/`, `perf/scenarios/`, `tools/shape-survey/configs/`, `tools/victoria-interop/logit-*.yaml`, `tools/splunk-interop/logit-*.yaml`); ordinary tests enforce this too |
