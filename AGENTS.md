...
`ComponentKind`s — not yet wired into `demo/logit.yaml`'s commented-out `trace_out` stanza, which
also needs a span producer before it's worth uncommenting (nothing emits a `SpanRecord` yet outside
tests). Config is a flat graph of named components (ADR 0009,
