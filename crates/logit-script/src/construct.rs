...
//! take the path as an argument so the metric (W3/W4) and span (W5) parsers of
//! `docs/plans/lua-event-constructor.md` can reuse them unchanged. `metrics` and
...
/// then its value goes through [`value_field`] so a bad value is `Event.new: <path>.<key> ...`
/// too. A nested table still converts through the shared `lua_to_value`, so a malformed value
/// *inside* one reports that helper's unprefixed attribute-conversion error.
fn attributes_from_table(t: Table, path: &str) -> mlua::Result<AttrMap> {
    ...
    for pair in t.pairs::<LuaValue, LuaValue>() {
        ...
        let Ok(key) = key.to_str() else {
            return Err(runtime_error(format!("Event.new: {path} has a non-UTF-8 key")));
        };
        map.insert(key, value_field(value, path, key)?);
    }
    Ok(map)
}
