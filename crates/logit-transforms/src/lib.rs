...
//! section). `aggregate`, `json`, `kv_metrics`, `keep`, `remove`, and `set` are implemented so
//! far; more (`logfmt`, `kv`, `regex`, `csv`, `rename`, `filter`, `sample`, `throttle`, `dedup`)
//! are expected to land here too.
