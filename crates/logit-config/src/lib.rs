...
pub struct Component {
    ...
    /// Per-sink delivery buffer (`docs/adr/0019-buffered-sink-delivery.md`). Meaningful only on a
...
pub struct BufferConfig {
    ...
    /// (`docs/adr/0019-buffered-sink-delivery.md`'s three-layer posture design). `Some(_)`
...
/// (`SinkQueueConfig::default`'s `64 * 1024 * 1024`). Always serializes back out as a bare integer
/// -- the simplest canonical form, and exactly what a config generated by `Serialize` (rather than
/// hand-written) should look like; a human still writes `64MiB` on the way in.
mod human_bytes {
