...
fn run_lua(
    ...
    loop {
        ...
        if errors > 0 {
            ...
            // Consistent with `write_loop`'s own rule (ADR 0022): the call site whose error path
...
mod tests {
    ...
    /// ADR 0022's rule that the call site whose own error path fired is the one that marks the
