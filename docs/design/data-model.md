...
| `statsd.timestamp` | `Value::Bool(true)` | Per-line marker `statsd_in` stamps alongside moving a `\|T<unix-seconds>` segment's value onto `Event::timestamp`, distinguishing a wire-supplied timestamp from a receipt-time one; `statsd_out` re-emits `\|T<secs>` only when this marker is set, under `format: dogstatsd` only. |
