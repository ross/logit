...
| `prometheus_in` decode 1 remote-write 1.0 request (100 gauge series) | **1525** | `remote_write::decode` alone -- Snappy and HTTP are the receiver's, and `families_to_events` is the scrape row's -- over `fixtures::remote_write_request_v1`. ~15.3/series, the same order as the scrape row above and for the same reason: labels become owned `String`s rather than slices of the request, one `Vec`/`HashMap` entry per series in the shared assembler, and one `Vec` per family. Prost decoding the request into owned protobuf structs is part of it; a borrowing decoder would be a different codec, not a tuning of this one. Not yet optimized, tracked as follow-up work alongside the two scrape/exposition rows |
| `prometheus_in` decode 1 remote-write 2.0 request (100 gauge series) | **1624** | the same request through 2.0's symbol table: **more** allocations than 1.0, not fewer. The table saves wire *bytes* by sending each string once, but the decoder still materializes one owned `String` pair per label (the model holds owned labels), and 2.0 adds a per-series reference-resolution pass on top -- +99, about +1/series. Worth knowing before reading 2.0 as the cheaper version: it is cheaper on the network and slightly dearer here |
...
| `prometheus_out` encode 100 series, remote-write 2.0 | **1434** | +211 over 1.0, about +2/series: the symbol table keeps an owned `String` key per distinct string, and this fixture's `shard` label values are distinct by construction, so it interns one per series. The trade is deliberate and the other half of it is on the wire, where 2.0 sends each of those strings once instead of once per series |
