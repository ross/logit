...
| `runtime.rs`, `write_loop` (`NextBatch`) | Unbiased: `store.peek()`, the grace arm (`unconstrained`) | The grace winning drops `peek` | `peek` reserves the head only in the poll that returns it, so a dropped `peek` reserved nothing. The queue is left for `finish_and_flush` |
