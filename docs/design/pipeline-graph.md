...
A bare file name is under `crates/logit-pipeline/src/` or `crates/logit-inputs/src/`, and
`outputs/` is `crates/logit-outputs/src/`.
...
| `runtime.rs`, `run_output` (sweep) | `timeout_at(SWEEP_DRAIN_TIMEOUT, inbox.recv())` on the closed inbox | The timeout firing drops a `recv` waiting on an upstream permit still reserved | The inbox is closed first, so a later send fails upstream as `events.dropped{reason="closed_consumer"}`. Every batch the sweep takes counts `batches.received`, then is spooled (`Disk`) or counted `dropped{reason="shutdown"}` (`Memory`). The bound covers only `recv`, never a disk `push`. A permit held past the bound is an uncounted exception (`docs/known-gaps.md`, "Pipeline runtime and graph") |
| `runtime.rs`, `write_loop` (`NextBatch`) | Unbiased: `store.peek()`, the grace arm (`unconstrained`) | The grace winning drops `peek` | `peek` reserves the head only in the poll that returns it, so a dropped `peek` reserved nothing, and neither arm needs priority. The queue is left for `finish_and_flush`. `unconstrained` keeps a sibling arm inside this `select!` that spends the coop budget from deferring the grace arm. It doesn't guard against `drain_inbox` in `run_output`'s unbiased outer `select!`: a `select!` polls no arm once the budget is spent, so a wake that polls `drain_inbox` first can leave the grace arm unpolled, and the deadline anchors within a few wakes of the signal, not one |
| `runtime.rs`, `write_loop` (`DeliverStep`) | `biased`: `deliver_with_retry`, then the grace arm (`unconstrained`) | The grace winning drops an in-flight `Output::send` or a backoff sleep | A cut-off send is `Fault::Ambiguous`: at-least-once leaves it queued, at-most-once commits it and counts `dropped{reason="shutdown"}`, and the span is tagged `fault=ambiguous`. A backoff sleep leaves the batch queued. A head left reserved is committed and counted by `SinkStore::finish` (`Memory`) or persisted at the read cursor (`Disk`). `biased` counts a send completing in the same wake as the deadline as delivered. Because the deliver arm is always polled first, `deliver_with_retry` checks the anchored deadline before every attempt and returns `Delivery::GraceExpired`, with no send started and the batch uncommitted, once it has passed: a batch reaching this step after the deadline, or a backoff ending at it, never starts a send the grace would then read as cut off. `unconstrained` has the same reach as in the `NextBatch` row |
| `runtime.rs`, `run_transform` | `timeout(wait, inbox.recv())` until the next flush deadline | The timeout firing drops a pending `recv` | `mpsc::Receiver::recv` is cancel-safe: a dropped `recv` takes nothing off the channel, and the next iteration receives it |
| `runtime.rs`, `revoke_lua_io` | `timeout(REVOKE_DRAIN_TIMEOUT, ..)` around a `recv` loop on the closed inbox, awaited by `watch_lua_thread` on a wedge and through `block_on` by `run_lua` on a failed loop | The timeout firing drops a `recv` waiting on an upstream permit still reserved | Every batch received counts `dropped{reason="shutdown"}` under the Lua node's id. A permit held past the bound is a named uncounted exception (ADR `shutdown-accounting-and-cancellation-safety`, decision 1; `docs/known-gaps.md`, "Lua") |
| `runtime.rs`, `deliver_with_retry` | `timeout(remaining, output.send(batch))`, the rest of the retry budget | The timeout firing drops an in-flight `Output::send` | The attempt is `Fault::Ambiguous` and counted `logit.component.errors`; `is_retryable` then retries it or `write_loop` counts it dropped under its reason. A sink's own state after a dropped `send` is the sink's to keep consistent: `stdio_out` and `file_out` can leave a torn line (`docs/known-gaps.md`, "File, stdio, and InfluxDB sinks") |
...
| `udp.rs`, `decode_loop` | `timeout(wait, pop_many)` until the next flush deadline | The timeout firing drops a waiting `pop_many` | `pop_many` awaits only on an iteration that removed nothing, so a dropped call took nothing. A grace-backstop drop mid-batch (an `emit` parked on a full downstream) is counted by the popped batch's `CountedDrain` as `datagrams.dropped{reason="shutdown"}`. The datagram whose `emit` is parked was already decoded and isn't counted; its events, and the accumulator's, are an uncounted gap in `docs/known-gaps.md`'s UDP intake section |
| `udp.rs`, `sample_while` | `biased`: `sleep(interval)`, then `read` | The sleep winning drops only the sleep; `read` is pinned outside the loop and re-polled | Nothing: `read_loop` resumes where it was. The sleep arm comes first so that a `read` exhausting the coop budget can't starve the sampler |
| `tcp.rs`, `TcpListener`'s accept loop | Unbiased: `accept` (TCP through `AcceptQueueSampler`, or `UnixListener::accept`), `shutdown.wait_for` | Shutdown winning drops an `accept` | `accept` is cancel-safe: a dropped call takes no connection off the kernel's accept queue |
...
| `tcp.rs`, `serve_connection` | `timeout_at(read_deadline, read_step)`, the earlier of the flush tick and the next-byte deadline | The deadline firing drops a `read_step` | `read_step` is cancel-safe (next row), so neither the flush tick nor the next-byte deadline loses stream bytes |
| `tcp.rs`, `read_step` | Unbiased: `read_buf`, `shutdown.changed()` | Shutdown winning drops a `read_buf` | `AsyncReadExt::read_buf` is cancel-safe: no bytes are consumed unless it returns, so this race and `serve_connection`'s `timeout_at` can drop it. The caller's `*shutdown.borrow()` check covers a shutdown that fired before the iteration |
| `tcp.rs`, `AcceptQueueSampler::accept_every` | `biased`: the sampling tick, then `listener.accept()` | The tick winning drops an `accept` | `accept` is cancel-safe, so a ready connection waits for the next turn. The tick comes first to match `udp.rs`'s `sample_while`; either order is correct here, because the loop returns to the synchronous sample on every accepted connection |
...
| `tail/driver.rs`, `Tailer::drain` (shutdown check) | None: `*shutdown.borrow()` between two files' reads, not a `select!` | Nothing is cancelled. Shutdown returns `DrainEnd::Shutdown` before the next file's read, and the run loop does the final close, flush, and forced checkpoint | One read is at most one 64 KiB chunk, and its decode is bounded, but shutdown isn't noticed while that chunk's lines are emitted, since an `emit` waiting on the downstream isn't raced against it. If `run_input`'s backstop drops the task there, the final flush and checkpoint don't run, and the restart replays from the last interval checkpoint: duplicates, never loss (`docs/known-gaps.md`, "File tailing and Docker logs") |
| `internal.rs`, `InternalInput::run_until_shutdown` | Unbiased: `ticker.tick()`, `shutdown_due` | Either arm winning drops the other before its body runs; a losing `tick` consumes no tick, and `wait_for` re-reads the value | Both arm bodies await: each calls `tick`, which drains the registry and awaits `Fanout::send`. While a body is parked on backpressure the `select!` isn't polled, so the signal is seen only once that send completes, and `run_input`'s backstop can drop the task with a drained batch parked in it. That batch is lost uncounted (`docs/known-gaps.md`, "Internal telemetry and self-logging") |
| `http.rs`, `drive_with_idle` (waits) | Three unbiased waits: `conn` against `activity.changed` while a request is in flight; `conn`, `sleep_until(deadline)`, and `activity.changed` while idle; and `conn` against `activity.changed` after the close grace | A `conn` win returns. Otherwise the loser is a `sleep_until` or a `Notified` | `conn` is pinned outside the loop and polled by reference, so it is never dropped. A dropped sleep loses nothing, since the loop recomputes the deadline from `last_progress`. `InFlight`'s `drop` stamps progress, then calls `notify_one`, which stores a permit when no `Notified` is waiting, so a request finishing between two waits still wakes the next one |
| `http.rs`, `drive_with_idle` (close grace) | `timeout(grace, conn)` after `graceful_shutdown` | The timeout firing drops nothing (`conn` is polled by reference); the loop then drops the connection only when no request is in flight | With nothing in flight, the drop discards only a stopped h1 head or an h2 handshake: no request was taken, and the client resends on the closed connection. A request in flight is waited out, so its response reaches the wire |
| `http.rs`, `collect_with_stall_bound` | `timeout(stall, body.frame())`, per frame | The timeout firing drops a pending frame read, and the body read so far | The request is answered `408` (gRPC `DEADLINE_EXCEEDED`) and nothing of it is sent downstream, so the client resends it |
| `prometheus.rs`, `scrape_target` | The `reqwest` request `timeout`, on each target's `JoinSet` task in `PrometheusInput::tick` | The timeout firing drops a scrape's request or its body read | The scrape is `ScrapeStatus::Timeout`, reported as `up` 0 for that target. A scrape consumes nothing at the target, so the next scrape reads it again: nothing is lost |
| TLS accept and first-byte bounds: `tcp.rs` `ConnectionSpawner::spawn`, `logit.rs` `LogitInput::run_until_shutdown`, `otlp.rs` `OtlpInput::run`, `datadog.rs` `DatadogInput::run`, `datadog_trace.rs` `accept_tcp` and `accept_unix`, `splunk.rs` `SplunkHecInput::run`, `prometheus.rs` `PrometheusReceiver::run` | `timeout(handshake_timeout, acceptor.accept(stream))`, or `timeout(handshake_timeout, stream.peek(..))` (`has_first_byte` on a Unix socket) | The timeout firing drops a TLS handshake or a `peek` | No request byte has been read: `peek` consumes nothing, and a handshake reads only TLS records. The connection closes, reported through the throttled `connection_error` diagnostic |
| `datadog_trace.rs`, `DatadogTraceInput::run` | Unbiased: `tcp_loop`, `unix_loop` | Either loop ending (a fatal accept error) drops the other accept loop | An accept loop holds no request: each connection runs on its own spawned task, which the drop doesn't reach |
| `logit.rs`, `LogitInput::run_until_shutdown` (accept) | Unbiased: `AcceptQueueSampler::accept`, `shutdown.wait_for` | Shutdown winning drops an `accept` | `accept` is cancel-safe, as in `tcp.rs`'s accept loop. Each connection task holds its own `shutdown` clone, so returning closes only the listener |
| `logit.rs`, `LogitInput::run_until_shutdown` (accept-error backoff) | `biased`: `absorb_accept_error`, then `shutdown.wait_for` | Shutdown winning drops the backoff sleep | The error is counted before the first await, as in `tcp.rs` |
| `logit.rs`, `serve_connection` | Unbiased: `read_header`, `shutdown.changed()` | Shutdown winning drops a `read_header`, with any header bytes already read | The connection is answered `Reject{GOING_AWAY}` and closed. `logit_in` writes `GOING_AWAY` only before a frame reaches `send_relayed`, so the peer knows the batch never landed and resends it. Only the header read races shutdown: a frame whose header has arrived is read, forwarded, and acked with no shutdown check |
| `logit.rs`, `handshake` | `timeout(handshake_timeout, ..)` around the `Hello` header and body reads | The timeout firing drops a partial `Hello` read | No data frame precedes `Hello`, so nothing is lost; the connection fails with `logit.proto.errors{reason="handshake"}` |
| `logit.rs`, `read_header` | `timeout_at(first_byte, read)` for the first byte, then `timeout(stall, read)` per read | The deadline firing drops one `read`, taking no bytes | `AsyncRead::read` consumes nothing unless it returns. An expiry is an idle close (`close_idle`, `connections.closed{reason="idle"}`) answered `GOING_AWAY`, so a partial header is resent by the peer |
| `logit.rs`, `read_frame_body` | `timeout(stall, read)` per read of the body | The timeout firing drops one `read`, and the body read so far | The frame is abandoned before `send_relayed`, the connection closes as idle with `GOING_AWAY`, and the peer resends the batch |
| `logit.rs`, `write_control` | `timeout(handshake_timeout, stream.write_all(..))` | The timeout firing drops a `write_all` of a control frame, possibly part-written | A stalled `Ack` ends the connection, counted `logit.proto.errors{reason="ack_write_stalled"}`: its batch already reached `send_relayed`, so a peer that resends it duplicates it, never loses it. A stalled `Reject` is abandoned on a connection already closing, counted `reason="reject_write_stalled"` |
| `outputs/prometheus.rs`, `serve` (`prometheus_out`'s exposition) | `timeout(response_deadline, serve)` per scrape connection | The deadline firing drops a connection mid-response | A scrape reads the registry without consuming it, so the next scrape gets everything; the scraper sees one failed scrape |
| Sink connect bounds: `outputs/logit.rs` `LogitOutput::connect_and_handshake`, and `connect` in `outputs/statsd.rs`, `outputs/syslog.rs`, and `outputs/graphite.rs` | `timeout(connect_timeout, ..)` around the TCP connect, the TLS handshake, and `logit_out`'s `HelloAck` read | The timeout firing drops a connect or handshake | No byte of the batch has left, so the attempt is `Fault::Clean`, and `deliver_with_retry` retries it under either posture |
