...
| `udp.rs`, `UdpListener::drive` | Unbiased: `read` (`read_loop_sampled`), `decode` (`decode_loop`) | Neither: the loser is awaited to the end | `read` finishing closes the queue, so `decode` drains it and returns; `decode` can't finish first while `read` runs. A grace-backstop drop of `drive` is counted by `ResidualOnDrop`, declared before both futures so it drops after them: it closes the queue and counts what it still holds as `datagrams.dropped{reason="shutdown"}` |
