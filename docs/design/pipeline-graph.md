...
17. A non-default `receive:` block is rejected on any kind that is not a **datagram listener**
    ...
    amendment: `syslog_in`/`graphite_in` under `transport: tcp`), or a **tail
...
43. A listener carrying a `tls:` block must be on a stream transport — `transport: tcp` on a
    `syslog_in` ([ADR `syslog-tcp-ingress-and-tls`](../adr/syslog-tcp-ingress-and-tls.md)) or a
    `graphite_in` ([ADR `graphite-carbon-relay`](../adr/graphite-carbon-relay.md)'s 2026-09-14
    amendment, which put that listener on the same driver). TLS is defined over a reliable ordered
    byte stream, and its datagram sibling DTLS is out of scope throughout this project (RFC 6012
    for syslog; carbon has no DTLS receiver at all) — so a `tls:` block under `transport: udp`
    ...
    **One rule, not one per listener.** The check, the message and the reasoning are identical on
    ...
    grows a stream transport joins this rule by adding an arm to its match (`statsd_in` is next)
    rather than by claiming another rule number — the opposite convention from the *sink* rules
...
45. A `handshake_timeout` must be greater than `0s` on every kind that has one — `syslog_in`,
    `graphite_in`, `logit_in`, `otlp_in` — and must be left at its default where it could never
    take effect: a `syslog_in` or a `graphite_in` with `transport: udp`.
    ...
    field instead of `tls:`, in rule 33's "only means anything under X" shape: a UDP `syslog_in` or
    `graphite_in` has no connection to hand shake at all, so an operator who set a value there
