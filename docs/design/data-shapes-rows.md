...
| Prometheus | labels added at scrape time | `job`,`instance` always, before relabeling; a typical `kubernetes_sd` job adds `namespace`,`pod` (or `service`) **plus one label per pod/service label** via `labelmap` | Reported / Counted | Default / Configured | | prometheus.io jobs_instances; prometheus@main `fixtures/prometheus-kubernetes.yml` | T5a#20-21 merge |
