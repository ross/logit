...
# `Arc<EventBatch>` copy-on-write on channels
...
guarantee (`docs/adr/multi-payload-events.md`, proven by
...
[ADR `multi-payload-events`](multi-payload-events.md) let one event carry a log and several metrics at once,
...
- **A routing primitive to avoid fan-out entirely** (named outlets, edge predicates, a pub-sub
  broker). Already considered and rejected by [ADR `component-graph-configuration`](component-graph-configuration.md):
