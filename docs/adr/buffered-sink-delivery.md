...
entire value comes from the consumer being asynchronous with respect to the producer. ADR `service-lifecycle-and-output-retry`
...
`docs/plans/buffered-sink-delivery.md` is the workstream breakdown; this ADR records the
...
`Arc::try_unwrap`/clone on a `Delivered::Shared` branch (ADR `arc-eventbatch-copy-on-write`'s copy-on-write design). A
...
**This revises, without superseding, ADR `service-lifecycle-and-output-retry`'s retry-budget decision.** 0013 set the default
...
closes ADR `service-lifecycle-and-output-retry`'s residual "no `Output` close hook" gap, now load-bearing since a sink can hold
