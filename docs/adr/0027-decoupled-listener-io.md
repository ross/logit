...
[`0026-relative-gauge-adjustments.md`](0026-relative-gauge-adjustments.md) — the same
...
entry was half-closed by [ADR 0021](0021-buffered-sink-delivery.md): `run_output` split into a
...
uncounted** — the failure mode ADR 0021's fix made rarer (a slow sink's own queue absorbs more
...
This ADR closes the listener half of the gap ADR 0021 left open, applying the same
decouple-the-I/O-from-the-work shape that ADR 0021 established, adapted to a producer (the kernel
...
- **`batch_max_events: 1` is the exact, magic-value-free spelling of "no accumulation."**
  ...
  byte the pre-ADR-0027 behavior. `0` stays rejected as an impossible bound (graph rule 18, the
...
- **Never merges across a resource change.** An accumulated batch carries one `Arc<Resource>`; if
  ...
  latent, the same *n*-to-1 hazard [ADR 0008](0008-aggregation-window-semantics.md) already
...
Today SIGTERM loses one in-flight datagram, and [ADR 0013](0013-service-lifecycle-and-output-retry.md)
...
The fix is an additive, defaulted trait method — the same shape ADR 0021 used for `Output::flush`:
...
The default body **is** ADR 0013's cancel-by-drop `select!`, relocated onto the trait from
...
**This revises ADR 0013, it does not supersede it** — the same treatment ADR 0021 gave its retry
...
completion. Three properties, each mirroring a bug ADR 0021's sink-side split hit and corrected in
review:
