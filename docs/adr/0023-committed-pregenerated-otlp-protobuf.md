# 0023 — Committed, pre-generated OTLP protobuf types; no `protoc` in any build path
...
[ADR 0004](0004-native-wire-format-with-otlp-bridge.md) settled OTLP as a first-class ingest/egress
...
`ExponentialHistogramDataPoint` alone). [ADR 0005](0005-containerized-development.md) says nothing
...
the exact tag/commit). `script/protogen` is a new `script/*` entry (ADR 0006's convention) that
...
- **`build.rs` + `prost-build` at build time.** Puts `protoc` on every build path, including the
  production release image (`Dockerfile`, not just `Dockerfile.dev`) — precisely what ADR 0005
...
- **This is the ADR 0004 qualification its own Consequences section flagged as possible but didn't
