...
its own risk, worked out separately in [ADR `aggregation-window-semantics`](aggregation-window-semantics.md)'s amendment;
...
pre-existing collision at `0020` (`demo-stack-separate-from-dev-stack.md` and
`trace-context-propagation-on-delivered.md`). By the time this branch merged with `main`,
`0024` had independently been taken by `hand-rolled-grpc-over-hyper.md` (and `0023` by
`committed-pregenerated-otlp-protobuf.md`) — a second instance of the same numbering-collision
