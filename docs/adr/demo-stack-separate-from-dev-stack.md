...
# A separate demo stack, not an extension of the dev stack

## Status
Accepted

## Context
`docs/plans/demo-stack.md` builds a "clone and `docker compose up`" path for someone with no
...
That stack's actual job, though, is the contributor dev loop (ADR `containerized-development`, ADR `scripts-to-rule-them-all`): a bind-mounted
...
`$DOCKER`/podman/sudo parity with the rest of `script/*` (ADR `scripts-to-rule-them-all`) — but `demo/README.md`'s
