# 0018 — Internal telemetry as ordinary pipeline events, drained from a component-level buffer
...
through `Delivered`, ADR 0017-gated) without a rename.
...
- A future `internal_spans`/log-carrying extension is additive to the same `internal` component —
  ...
  ADR 0017) is answered on its own evidence.
