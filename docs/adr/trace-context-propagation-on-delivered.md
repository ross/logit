...
trade ADR `minimize-allocations-over-event-size` actually settled), zero change to any allocation-count assertion, no attributable
...
- **`run_output`.** Already borrows `&Delivered` without unwrapping (`Output::send(&EventBatch)`,
  [ADR `arc-eventbatch-copy-on-write`](arc-eventbatch-copy-on-write.md)), so the incoming context is already there to
...
- **No `SpanRecord` is emitted anywhere.** Propagation gets the *right* context threaded to the
  *right* place; nothing yet decides when a node's visit to a batch becomes a real
  ...
  ([ADR `internal-telemetry-as-pipeline-events`](internal-telemetry-as-pipeline-events.md) already named it free to grow this
