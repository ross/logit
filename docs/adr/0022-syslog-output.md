# 0022 — Syslog egress: format, transport, and header-field precedence
...
`ComponentKind` (`docs/plans/0003-demo-stack.md`'s gap table). That gap is the specific reason the
demo stack's log leg (`demo/compose.yaml`'s `loki` and `alloy` services) has been standing up
provisioned and unfed since `docs/plans/0003-demo-stack.md` landed — `AGENTS.md` names
...
(`0020-demo-stack-separate-from-dev-stack.md` and
`0020-trace-context-propagation-on-delivered.md`) from an earlier merge. `0022` is simply the next
...
TCP is what makes `Fault` classification (`docs/adr/0021-buffered-sink-delivery.md`) meaningful for
...
`docs/adr/0021-buffered-sink-delivery.md`'s sustained-permanent-failure exit window.

**A bounded reconnect exception to "one attempt per `send`."** ADR 0021 makes `Output::send` a
...
retries (ADR 0021's table) — covering the common outage shape (the receiver restarting) with zero
...
- **RFC 3164 as the default format.** Rejected: its ambiguous timestamp is a real cost for a demo
  ...
  arbitrary narrowing.
- **UDP only.** Rejected: it leaves ADR 0021's `Fault` classification almost vacuous for this sink
...
- **`log.severity` outranking `syslog.severity`.** Rejected: it would silently rewrite, on every
  ...
  a relay's job.
- **A single fixed delivery guarantee, ignoring transport.** Not applicable here beyond ADR 0021's
