...
# `kv_metrics`: skip rules, numeric coercion, and no `tags:` field
...
`docs/plans/nginx-integration.md`'s workstream E needs a way to turn attributes already on an
...
diagnostic (ADR `json-parsing-into-attributes`), a field being absent or non-numeric on some fraction of events is expected
...
- **Rejecting a non-numeric field as a hard pipeline error.** Rejected outright, for the same reason
  every other transform in this codebase avoids dropping telemetry over one bad value (ADR `aggregation-window-semantics`, ADR
