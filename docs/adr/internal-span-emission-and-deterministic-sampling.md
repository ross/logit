...
[ADR `trace-context-propagation-on-delivered`](trace-context-propagation-on-delivered.md) put a real `TraceContext` on every
...
decoupled from delivery through a `SinkQueue`** ([ADR `buffered-sink-delivery`](buffered-sink-delivery.md)).
...
- `run_flush` used to mint a fresh root **per resource group**. It now mints **one root before
  ...
  per-resource windowing ([ADR `aggregation-window-semantics`](aggregation-window-semantics.md)), not *N* hops. The
...
Follows [ADR `internal-telemetry-as-pipeline-events`](internal-telemetry-as-pipeline-events.md)'s shape: spans ride the same
...
`Vec<Event>`, and span events ride the same batch as metric events. That is exactly what ADR `internal-telemetry-as-pipeline-events`'s
...
- **A propagated `sampled` flag on `TraceContext`, OTel-W3C-style.** Rejected in favor of a
  ...
  grew once, per ADR `trace-context-propagation-on-delivered`) for no benefit here — a deterministic-on-`trace_id` sampler gets the same
...
- `size_of::<Delivered>()` stays exactly 56 (unchanged from ADR `trace-context-propagation-on-delivered`) — the whole point of the
