...
# `Event` carries a log, metrics, and a span at once, not one of the three
...
`kv_metrics` transform (`docs/plans/nginx-integration.md`'s workstream E) that needs to add
...
outright here: pre-1.0, no external config or script users exist yet (the same reasoning ADR `component-graph-configuration`
...
- **ADR `aggregation-window-semantics`** (aggregation window semantics): pass-through is now per-*metric*, not per-*event* —
  see that ADR's amendment section.
- **ADR `json-parsing-into-attributes`** (JSON parsing into attributes): "only `Payload::Log` events... are candidates" no
...
- ~80 call sites across `logit-core`, `logit-script`, `logit-pipeline`, `logit-inputs`,
  ...
  (see the ADR `aggregation-window-semantics` amendment and `crates/logit-outputs/src/influxdb.rs`'s `render_tag_suffix`
  split, respectively).
