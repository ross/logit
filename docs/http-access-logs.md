...
How to point a web server's access log at `logit` and get OpenTelemetry-conformant HTTP semantic
conventions out the other end — a real trace span, a bounded metric set, and a log record — without
writing a `map`/`log-format` normalization layer on the server side. This is the operator-facing
schema for the `http_access` transform; see
[ADR `http-access-normalization`](adr/http-access-normalization.md) for why the work lives in the
pipeline rather than on the server, and
[`docs/plans/http-access-normalization.md`](plans/http-access-normalization.md) for the build-out.
[`docs/deploying.md`](deploying.md)'s "The nginx-side recipe" covers the operational side of
running `logit` against a real server (keeping the old log line during cutover, the syslog
message-size limit, the startup-ordering rule); this doc is the schema half that recipe points at.

## What this is

Historically every deployment re-derived the same access-log normalization by hand, in the
server's own config language: rename each variable to its OTel semantic-convention name, cap every
free-text field, turn `HTTP/1.1` into `1.1`, turn a `000` client-abort status into a real integer,
classify the user agent into `browser`/`crawler`/`tool`/`scanner`/`other`, classify the request
path into a handful of low-cardinality routes for a span name, decide when a status becomes
`span.status: error`, and redact sensitive query parameters. A real nginx deployment does this in
roughly a hundred lines of `map` blocks — correct, defensible, and something every nginx user (and
every HAProxy, Varnish, or Squid user, in a different language with different escaping rules) has
to rediscover. It also fails quietly: an unquoted `"status":$status` produces invalid JSON
...
The contract here is simpler: **the server logs the raw, untouched value under the standard OTel
semconv attribute name, and nothing else** — no capping, no coercion, no classification, no
escaping beyond making the line valid JSON. One `http_access` component, placed once in the
pipeline, does the rest: coercion, unit merging, method/version normalization, query redaction,
length capping, control-byte cleaning, user-agent and route classification, and span derivation.
Sending raw semconv names rather than a `logit`-specific namespace means a pipeline without
`http_access` still carries standard attribute names (just unhardened), anyone reading the config
recognizes every key, and the mapping from a server's own variables to the schema is this doc,
not code. There is deliberately no `preset: nginx`: the component would then own every server's
variable vocabulary and its drift.

nginx, Apache, HAProxy, Varnish, Squid, and Envoy all let you choose the key each value is logged
under, so they write the schema directly. Caddy and Traefik don't — their JSON access logs have
fixed key names — so for those two the pipeline renames first (see their snippets below).
...
Every `http_access` field is optional: a bare `type: http_access` still coerces, caps, classifies
user agents with the built-in table, and derives the span fields. It rewrites attributes only on
an event that carries a log record — a metric or span event passes through untouched — and it
never touches `event.log`, `event.metrics`, `event.span`, `event.timestamp`, or the batch
`Resource`. It never drops an event: a value that doesn't parse is left exactly as it arrived and
counted, while every other field is still normalized.
...
"what `http_access` does" column; you never do any of it server-side. **Absent rule, across every
field:** `""`, `"-"`, and a JSON `null` all count as absent — the rule
[`docs/design/data-model.md`](design/data-model.md)'s well-known-attribute table and
`trace_context` already use — so an unset HAProxy `txn` var, an unpopulated nginx `$upstream_*`
variable, and a JSON `null` read the same. An absent field produces nothing: no default
`url.scheme`, no `user_agent.class` for a producer that doesn't log the header, no fabricated
route.
...
and the unsuffixed form. The unsuffixed form must be an integer (or a quoted one) — a fractional
...
| Attribute | Send | What `http_access` does |
|---|---|---|
| `user_agent.original` | string, raw header value | Classified into `user_agent.class` on the **uncapped** value (a spoofed UA's identifying token is often at its tail), then capped (256 chars). Absent: no class. Present but `""`/`"-"`: class `none`. |
...
Two names are `logit`'s own: a server often logs one field where semconv wants several.
`http_access` decomposes each into whichever atomic fields are **absent** — a composite never
overrides an atomic field you also sent — then removes the composite.
...
any `span.start*`/`span.end*` attribute — those are [`data-model.md`](design/data-model.md)'s
existing well-known table, and `trace_context` reads them exactly as documented there, including
its rule that only one form of a timing quantity may be present. The one thing worth repeating
here is which end of the request each server's timestamp marks, since sending an end as a start
silently produces a span shifted by its own duration.
...
`span.duration_s` mirror — `(end − duration, end)` for an end-stamped server, `(start, start +
duration)` for a start-stamped one — without logging a span duration separately.
...
request. If a `traceparent` header arrived, log it verbatim under `traceparent` and
`trace_context` takes the trace id, this line's parent span id, and the flags from it. If none
arrived — this server is the edge — mint one at the edge and log it as `trace.id`. nginx already
mints a per-request id (`$request_id`, 32 hex characters); this `map` uses it as a fallback while
still reusing an inbound trace id:
...
Log `$trace_id` under `trace.id` alongside the raw `traceparent`. An explicit `trace.id` always
wins over the header's trace id in `trace_context`, which is why the regex arm matters: with only
`default $request_id`, every request would start a new trace even when an inbound one arrived. If
the tier also forwards a `traceparent` to its backend, it has to log the span id it put in that
header, which is what [`demo/nginx/nginx.conf`](../demo/nginx/nginx.conf)'s maps do.

## Quoting rules for nginx `escape=json`

`escape=json` escapes `"`, `\`, and control bytes correctly, but which variables are wrapped in
`"..."` is up to you. Getting it wrong either breaks the line's JSON or loses precision:

- **Quote every variable that can be unset, empty, or non-numeric on some code path.** An
  `$upstream_*` variable renders `-` or `""` on a non-proxied vhost; a bare `-` is not valid JSON.
- **Always quote `$status`.** nginx can log a literal `000` on an abnormal termination
  ...
  **not valid JSON**, and the whole line is lost. This repo's own nginx configs had exactly this
  bug until `http_access` landed. `http_access` turns the quoted `"000"` into `0`.
...
- **Set `invalid_utf8: replace` on the `json` component ahead of `http_access`.** `escape=json`
  ...
  but `json`'s strict parse fails the whole line — the one failure `http_access` can't reach from
  behind `json`, since no attributes are left to normalize. Under `replace`, a parse that failed on
  invalid UTF-8 is retried on a copy with every invalid sequence replaced by U+FFFD; a well-formed
  line's cost is unchanged. The default stays `reject`.

## Emitters that can't write dotted keys

HAProxy's own JSON encoder (`%{+json}o`, HAProxy 3.0+) names each item with `%(name)`, and that
name grammar accepts only `[A-Za-z0-9_-]` — a literal `.` is rejected (verified against
HAProxy's `src/log.c` name parser). So **every canonical name in this doc is also accepted with
each `.` replaced by `-`, and only the `.`**: underscores stay where they are. That gives
`http-request-method`, `url-path`, `http-response-status_code`, `user_agent-original`,
`span-start_us`, `http-request-duration_ms`, and `http-request-header-x-forwarded-for` (the
header key keeps its own dashes). It is `-`, not `_`, because canonical names already contain `_`
(`user_agent.original`, `span.start_us`), so an underscore spelling couldn't be decoded from the
dotted name by rule; the dashed one can, by eye.

The alias table is fixed — every name `http_access` reads or writes, plus the trace/timing names
(`trace-id`, `trace-flags`, `span-id`, `span-parent_id`, `span-kind`, `span-end_s`, …), since
`http_access` runs before `trace_context` and `trace_context` itself only reads dotted names. It
is never a blanket `-`→`.` rewrite, which would corrupt a legitimately dashed key. `http_access`
renames each dashed key to its dotted spelling before anything else runs; if both spellings
arrive, the dotted one wins and the dashed key is removed either way. A name with no dots
(`traceparent`) needs no alias. A non-canonical extra field of your own has no alias entry — give
it an underscore name (`haproxy_timer_queue_ms`), never a dash standing in for a dot, so it can't
collide with a future canonical name.
...
Each directive below was checked against the vendor's own docs or source; a few lower-confidence
items are called out inline. The nginx and HAProxy formats are the ones this repo ships, checked
with `nginx -t`/`haproxy -c` against the real images and verified with live lines. The others have
not been run against a live instance, so treat "cannot provide" as researched, not measured, and
check anything that matters to you against your deployed version.
...
Optionally add `"http.response.compression_ratio":"$gzip_ratio"` — quoted, since it is empty
unless that response was actually gzip-compressed. `$remote_port` is quoted because it is empty
for a UNIX-socket listener.
...
header — nginx does no XFF trust logic of its own.
...
`%U%q` as `url.original` — `%r` is exempt. `%{REQUEST_SCHEME}e` is lower-confidence (verified
indirectly, not on `mod_log_config`'s own page).
...
pipe; point `tail_in` at the file, or pipe to `logger`.
...
a named item. A bracketed `[expr]` fetch of a request header is rejected in a `log-format` ("may not
be reliably used here"), so Host, User-Agent, Referer, XFF, `traceparent`, and the scheme are
captured into `txn` vars first. `%si` is the backend IP HAProxy connected to; `%s` would be the
...
because client-mode `Varnish:time_firstbyte` is end-to-end time to first byte, populated on cache
hits too:
...
`%{usec}t` really is microseconds since the epoch (checked in `format_time()`'s source; the docs'
prose calls it milliseconds).
...
**Gotchas:** a VSL buffer overrun under sustained load silently drops a *gap* of lines — no
backpressure, no retry. Pass `-g request` explicitly (the compiled-in default is `vxid`).
...
"no response") — keep it quoted; `%rv` has no `HTTP/` prefix, so type it literally; many codes log
`-` for "not applicable".
...
(checked against `caddyserver.com/docs/logging` and Caddy's `server.go`/`marshalers.go`):
...
So the pipeline renames instead: `json` decodes the object, `flatten` dot-joins it
(`request.method`, and `request.headers.User-Agent.0` — every header value is an array), and a
`lua` stage moves each field onto its canonical name. `logit` has no native rename component (`set`
only stamps constants, and the old `rename` kind was retired in favour of Lua,
[ADR `routing-by-condition-is-lua`](adr/routing-by-condition-is-lua.md)). Writing `nil` stores a
null, which `http_access` treats as absent and a later `keep` removes:
...
`duration` is already seconds, the one field needing no unit at all. `request.uri` is Go's raw
`RequestURI`, query included. A Lua stage costs a VM per worker and a few allocations per event
(`docs/design/memory.md`) — the price of a server whose key names can't be chosen.
...
Fixed key names like Caddy, but flat — `json` plus a `lua` rename, no `flatten`:
...
the Lua hop: any value below 2^53 is an exact Lua integer, and a request would have to take over
100 days to exceed it.
...
Ordered rules over the *capped* `url.path`; first match wins. Each entry is exactly one of a named
built-in set or a `match` regex with a literal `route` value — never a capture, so every
`http.route` comes from config or the table below. A `match` is unanchored unless it anchors
itself:
...
`probes` and `well_known` are case-sensitive because those paths are protocol- or
convention-mandated literals (`robots.txt` is lowercase by spec, `/healthz` by Kubernetes
convention); `/ROBOTS.TXT` is a different, usually 404, route. A file extension's case carries no
meaning, so `assets` ignores it. `assets` deliberately excludes `json`/`xml`/`txt`/`csv`: those
are routinely API responses, and routing one to `/{asset}` would hide it. Built-ins expand in
place, so a rule's position in the list sets its priority. A `builtin:` may appear once.
...
Extra classes, tried in order **before** the built-in table — the built-in table can be pre-empted,
never disabled:
...
`browser` is last because crawlers routinely claim `Mozilla/`. A UA matching nothing is `other`.
The tables were checked against a corpus of 62 real, sourced user-agent strings and 18 real paths.
A few choices worth knowing: `\bbot\b` rather than `bot\b`, which would match phone models like
`CUBOT`; bare `bot/` alongside it to catch `DotBot/1.2`, `Discordbot/2.0`, and other `…Bot/`
tokens with no word boundary before them (see `docs/known-gaps.md` for the trade-off);
`uptimerobot` in `tool` both because it is a synthetic monitor and because `bot/` would otherwise
catch `UptimeRobot/2.0`; `yandex` rather than `yandexbot`, since most of Yandex's robots carry no
`bot` token; `^java/` anchored, because Java's `HttpURLConnection` sends exactly `Java/<version>`;
`fuzz faster u fool` because ffuf's default UA contains no "ffuf". Scanners that spoof a browser by
default classify `browser` — nikto 2.6.1+ defaults to a browser UA, and nuclei and Nessus do the
same — so the `scanner` row catches only a scanner that identifies itself.
...
character boundary, so a capped value stays valid UTF-8; then any control byte (`< 0x20`, `0x7F`)
left in the value becomes `_`. A value that arrived as raw, non-UTF-8 bytes is capped in bytes
instead.
...
trusted-proxy list or hop count (see "What it does not do"). `{trust: false}` is rejected; omit
the block instead, so there's one spelling of "off".
...
closed range, so they can go straight into an `aggregate` series key or a span name. It does not
bound what it only caps: `server.address`, `client.address`, `upstream.address`, `cache.status`,
and the rest are bounded in *length*, not in *value*. A `Host` header is attacker-controlled; left
unclamped, a stuffed one becomes its own series. Clamping a value set is `keep_values`' job
...
And `keep` still goes immediately before `aggregate`: `aggregate`'s `SeriesKey` includes every
attribute, so whatever survives `keep` sets both series cardinality and per-window memory. Keep
`url.path`, `client.address`, and every `user_agent.original`-style free-text field out of it.

## Metrics

`http_access` emits no metrics itself. It guarantees the attribute names semconv's
`http.server.request.duration` and its low-cardinality attribute set expect; a stock `kv_metrics` +
`keep` pair produces the metric:
...
Start `logit` and wait until it is listening before pointing the server at it — UDP is
fire-and-forget, and a line sent before the listener is bound is lost with no error anywhere:

```sh
until logit ready --admin http://logit-host:9600; do sleep 0.5; done
```

`docs/deploying.md`'s "The ordering rule" and "Probes and exit codes" sections cover the fallback
when `admin:` isn't configured.

Once traffic is flowing, watch:

- **`logit.transform.http_access.routed{outcome}`** — `rule`, `builtin`, `other` (fell through to
  `route_other`), or `none` (no rule and no `route_other`). A large or climbing `other`/`none`
  share means your `routes:` list is missing a real route; the aim is a trickle of genuinely
