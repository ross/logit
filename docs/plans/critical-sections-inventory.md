...
| [RT-11](#rt-11--lua-node-hosting-os-thread-two-oneshot-handshake-catch_unwind-handleblock_on) | P0 | Lua node hosting: OS thread, two-oneshot handshake, `catch_unwind`, `Handle::block_on` | `runtime.rs` (`run_lua`, `watch_lua_thread`, `run_lua_loop`) | in-progress (luab/w3) |
...
- **Invariants to verify:**
  - **The borrow-then-release ordering.** `AttrsProxy::__newindex` takes `this.0.borrow()` for the no-op check, drops it, then calls `lua_to_value` (which can re-enter a script `__index` via `Table::get`), then takes `borrow_mut`. Verify the same ordering holds in `ResourceProxy`'s `__newindex` (`resource.rs`) and `ScopeAttrsProxy`'s `__newindex` (`scope.rs`), and that a re-entrant write during `lua_to_value` can't produce a lost update or a `RefCell` panic. **Holds, and the re-entry path is gone (#385):** the ordering is the same in all three, and conversion now reads every table raw (the array branch moved from `Table::get` to `raw_get`; the map branch's `pairs` was already `lua_next`), so no metamethod runs during it. `a_metamethod_that_writes_back_into_the_event_never_runs_during_an_attribute_write` assigns a table whose `__index`/`__newindex` write into the same event and checks that nothing is written. With `newproxy` removed in `luab/w2`, a script has no way to run code during an allocation either.
...
- **Verdict (findings → #385):** the depth and raw-read leads were driven by adversarial scripts through a real `ScriptWorker` (the seven `depth_tests` in `value.rs`); both cyclic cases aborted before the cap and error after it. The 2^53 boundary was checked through the generated events of `crates/logit-script/tests/event_new_fixed_point.rs` rather than a separate `lua_value_matches` proptest, which found one new residual: mlua 0.9.9 reads a LuaJIT number within `f64::EPSILON` of an integer as that integer, so a nonzero float below 2^-52 written back through a proxy is stored as `I64(0)` (`a_float_below_epsilon_written_back_becomes_zero_a_recorded_residual`, recorded in ADR `lua-event-constructor`'s count amendment).
...
- **Verdict (findings → #385):** the proptest and the pinned-source check were run. F8 and the tiny-float residual are the two findings: the first is fixed, the second recorded. Unbounded interning from `name`/`unit`/`description`/`event_name` stays with CORE-19 (`luab/w5`).
