...
1. **Nothing produced a span.** The PR series ending at #42 built the *substrate* only — ADR `trace-context-propagation-on-delivered`
...
`demo/` ([demo-stack.md](demo-stack.md)) was the forcing function: Tempo stood up,
...
- **An ADR per real decision**, numbered, in `docs/adr/`, following the existing
  ...
  own real `syslog-output.md` first, so PR1's own ADR (internal span emission) renumbered
  ...
  was added.
- **Every config type derives `Serialize + Deserialize + JsonSchema` together** (ADR `config-yaml-jsonschema`), and
...
windowing (ADR `aggregation-window-semantics`), not N hops. And the listener span's window is the `send` call, not decode —
...
**The emit API and the buffer**, following ADR `internal-telemetry-as-pipeline-events`'s shape — spans ride the same `ComponentBuffer`
...
a new, deliberately non-`cibuild` script (`script/protogen`, per ADR `scripts-to-rule-them-all`) that builds a one-shot
image with `protoc`; **no drift check in `script/cibuild`** — that would put `protoc` in the CI
image, exactly what ADR `containerized-development` forbids. See [ADR `committed-pregenerated-otlp-protobuf`](../adr/committed-pregenerated-otlp-protobuf.md).

**`Decoder`/`Encoder` gained a sibling, not a change** — OTLP has three services, and one
`EventBatch` can hold all three signals on one `Event` (ADR `multi-payload-events`), which the existing
...
interner). See [ADR `hand-rolled-grpc-over-hyper`](../adr/hand-rolled-grpc-over-hyper.md).
...
`otlp_out` retries through `write_loop` like any sink regardless — ADR `service-lifecycle-and-output-retry`'s ordering rule).
...
sustained-permanent-failure guard ([ADR `service-lifecycle-and-output-retry`](../adr/service-lifecycle-and-output-retry.md),
revised by [ADR `buffered-sink-delivery`](../adr/buffered-sink-delivery.md)) killed the entire `logit` process
