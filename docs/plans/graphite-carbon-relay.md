...
#170, W4a #171, W4b #175; W4a/W4b opened alongside each other, both branched from
`feat/graphite-w2` merged with `feat/graphite-w3`), **but the stack is not yet merged to `main`**
-- per Ross's direction (several efforts are in flight at once), every PR targets its parent
branch and retargets to `main` only once that parent merges, so "landed" here means "complete and
pushed," not "in `main`."
