...
| Retry budget | widens from 5s to 60s (ADR `service-lifecycle-and-output-retry`'s ~5s was deliberately tight *because* delivery wasn't decoupled) |
...
| `Output` has no close/flush hook | ADR `service-lifecycle-and-output-retry`'s residual gap; becomes load-bearing once a sink can hold unwritten data at shutdown |
...
**Decisions to record:** draft **ADR `buffered-sink-delivery`** here (Status: Proposed) so later workstreams can cite
...
`docs/design/wire-protocol.md`, `docs/adr/buffered-sink-delivery.md` (new, drafted here).
...
> wrong, both corrected during review and recorded in `docs/adr/buffered-sink-delivery.md`'s
...
> `docs/plans/nginx-integration.md`'s workstream D precedent for the same situation.
...
**Decisions to record:** finalize **ADR `buffered-sink-delivery`** to Accepted, including the failure-handling
decision below; add a short "Revised by ADR `buffered-sink-delivery`" note to ADR `service-lifecycle-and-output-retry`'s retry section (0013 is not
...
pub trait Output {
    ...
    /// follow. Default no-op — closes ADR `service-lifecycle-and-output-retry`'s residual "no close hook" gap without touching
