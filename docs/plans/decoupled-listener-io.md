...
doesn't re-derive design that belongs in [ADR `decoupled-listener-io`](../adr/decoupled-listener-io.md) or a
design doc.
...
invent — see [ADR `decoupled-listener-io`](../adr/decoupled-listener-io.md)'s Context section for the full
...
transforms, Lua nodes and sinks ([ADR `service-lifecycle-and-output-retry`](../adr/service-lifecycle-and-output-retry.md)).
...
### Shutdown drain must not regress ADR `service-lifecycle-and-output-retry`'s accepted loss

Today SIGTERM loses one in-flight datagram, and ADR `service-lifecycle-and-output-retry` accepts that explicitly on the premise
...
([ADR `decoupled-listener-io`](../adr/decoupled-listener-io.md) has the full design) is an additive, defaulted
trait method whose default body **is** ADR `service-lifecycle-and-output-retry`'s own `select!`, relocated onto the trait, so a
...
*revises* ADR `service-lifecycle-and-output-retry`'s rejection rationale for cooperative shutdown; it does not supersede the ADR.
...
exactly as designed in [ADR `decoupled-listener-io`](../adr/decoupled-listener-io.md). `NodeSpec::Input` becomes
...
> it was holding, uncounted — bounded to exactly one, the same scope ADR `service-lifecycle-and-output-retry` already accepted.
...
[ADR `decoupled-listener-io`](../adr/decoupled-listener-io.md) for the numeric derivation against the field's own
tuning figures.
...
[ADR `decoupled-listener-io`](../adr/decoupled-listener-io.md) already describe.
