...
never lands in the checked-in YAML. W1's kernel-drop visibility gets the cross-check against `/proc/net/udp` described in its own
table row — manual at first, automated as an exact-equality test by `libc/w2` (the count is not
deterministic, but the two counters are the same `sk_drops`, so equality between them is).
