...
- **Cross-protocol semantic gaps.** OTLP (`crates/logit-proto/src/otlp/`) is `logit`'s first
  *second* wire model, and its codec is the first place "our internal model can't cleanly express
  ...
  | non-OTLP sinks / `aggregate` | A `MetricRecord` flagged `NO_RECORDED_VALUE` → skipped at `influxdb_out`/`statsd_out`, rendered as `no_recorded_value` at `stdio_out` (never dropped — a debug sink must show it), passed through unmerged at `aggregate` | `logit.output.messages.dropped{reason="no_recorded_value"}` (statsd) / throttled diagnostic key `no_recorded_value` (influxdb) / `logit.transform.metrics.passed_through{reason="no_recorded_value"}` (aggregate) | Only `otlp_out` can keep a flagged point on the wire and re-encode it unchanged — that's the fixed point `docs/adr/lossless-transit.md` requires for `otlp_in -> otlp_out`. No other sink or transform has a wire/model concept of "no value here," so treating the flag's default numeric payload as a genuine reading would fabricate a sample the producer never sent (`crates/logit-core/src/metric.rs`'s `flags` doc). |
  ...
  | encode (Prometheus) | Name/label sanitization: every byte outside `[a-zA-Z0-9_:]` (metric) / `[a-zA-Z0-9_]` (label) → `_`, a leading digit → `_` prefix; two labels colliding after that keep the one whose original name sorts first, and an attribute colliding with a generated `le`/`quantile` is dropped | `logit.output.labels.dropped{reason="collision"\|"reserved"}` (a metric-name collision is uncounted) | Substitution, not deletion, is the `statsd_out` precedent (`crates/logit-outputs/src/statsd.rs`'s `sanitize_into`): distinct inputs stay distinct in the common case. Collisions are the residue — `a.b` and `a-b` are one label name on the wire, and two *metric* names that collide become two `# TYPE` lines for one name, which is the operator's own naming clash, not something a codec can resolve. Prometheus 3's quoted UTF-8 names would remove the need for most of this; supporting them is a tracked follow-up. |
