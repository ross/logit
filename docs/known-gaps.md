...
- **`otlp_out` has no compression and no gRPC TLS** —
  ...
  Tempo and plaintext HTTP to Loki); both block a real deployment. The compression half of this
  mirrors the `otlp_in` gap above but was never itself filed until now.
