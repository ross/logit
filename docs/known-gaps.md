...
- ~~**`logit_proto::Encoder`'s single-`Bytes`-per-batch contract doesn't fit a sink that needs
  ...
  state set once per transport). Follow-up, not done here: the collectd codec adopting it in
  place of its own `Packets` buffer when that branch lands. `prometheus_out` stays outside all
  three traits by design.
