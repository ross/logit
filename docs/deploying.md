...
- **Exit code `2` naming a component "still inside process()/flush() ... after shutdown began"**
  ...
  still drains and flushes normally and anything sent to it afterwards is counted as dropped, then
  exits without it. 2 s is shorter than a sink's 5 s `buffer.shutdown_grace`, so a window flushed
